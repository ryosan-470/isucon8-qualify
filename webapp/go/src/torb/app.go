...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	rows, err := db.Query("SELECT S.id, S.rank, S.num, S.price, R.id, R.user_id, R.reserved_at FROM (SELECT * FROM sheets ORDER BY `rank`, num) as S LEFT OUTER JOIN (select * from reservations WHERE event_id = ? AND canceled_at IS NULL GROUP BY event_id, sheet_id HAVING reserved_at = MIN(reserved_at)) as R ON R.sheet_id = S.id", eventID)
	if err != nil {
		log.Fatal(err)
		return nil, err
	}
...
func getAPIUsers(c echo.Context) error {
	var params struct {
...
func getAPIReserveEvent(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func deleteAPIReservation(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func postAdminEditEvent(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getAdminEventSalse(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getAdminSalse(c echo.Context) error {
...
func main() {
	...
	e.POST("/api/events/:id/actions/reserve", getAPIReserveEvent, loginRequired)
	e.DELETE("/api/events/:id/sheets/:rank/:num/reservation", deleteAPIReservation, loginRequired)
